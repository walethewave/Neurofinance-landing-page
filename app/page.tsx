...
export default function NeuroFinanceLanding() {
  ...
  const handleDemoSubmit = async () => {
    ...
    setTimeout(() => {
      const responses = {
        revenue:
          "📊 Based on your daily tracking data: Revenue trend shows 15% MoM growth. Current run rate: $2.4M ARR. Recommendation: Optimize pricing for enterprise tier.",
        cash: "💰 Cash runway analysis: 18 months at current burn rate. Daily tracking shows $12K average burn. Recommendation: Reduce customer acquisition cost by 20%.",
        pricing:
          "📈 Pricing scenario analysis: 20% increase would boost revenue by $480K annually. Customer churn risk: 8%. Net impact: +$441K. Recommendation: Implement gradually.",
        market:
          "🌍 Market expansion analysis: European market shows 3x higher LTV. Entry cost: $150K. Break-even: 8 months. Recommendation: Start with UK market.",
        funding:
          "💼 Funding comparison: Debt financing at 8% vs equity at 25% dilution. For $1M: Debt saves $250K in equity value. Recommendation: Debt financing optimal.",
      }

      const key = Object.keys(responses).find((k) => demoInput.toLowerCase().includes(k)) || "revenue"
      setDemoResponse(responses[key as keyof typeof responses])
